...
proofCheckGroups :: Tag t => ProofCheckGroup t a -> [(ProofCheckGroupCheckIndices, ProofCheckGroup t a)]
proofCheckGroups = toList . proofCheckGroupsWithKeys

proofCheckGroupsWithKeys :: Tag t => ProofCheckGroup t a -> Map CheckGroupKey (ProofCheckGroupCheckIndices, ProofCheckGroup t a)
proofCheckGroupsWithKeys checks = M.unionsWith (<>)
    [ M.singleton (compatKey (groupKey check)) (ProofCheckGroupCheckIndices (S.singleton i), [check])
