...
pruneProofCheck analysis = over checkVisits pruneVisitWithTag
  where
    pruneVisitWithTag (WithTag tag (VisitCompat nodeId restrs)) =
        WithTag tag (VisitCompat nodeId (filter (testRestr tag) restrs))
...
getVisitWithTag :: MonadChecks t m => t -> NodeId -> m (WithTag t VisitCompat)
getVisitWithTag tag n = WithTag tag . VisitCompat n <$> getRestrsForTag tag
    ...
    let eqSide side = do
            ...
            visit <- case side.quadrant.direction of
                PairingEqDirectionIn -> do
                    ...
                    return $ VisitCompat entryPoint []
                PairingEqDirectionOut -> do
                    VisitCompat Ret <$> getRestrsForTag tag
...
getVisitToRestrPointAfter :: MonadChecks t m => RestrProofNode t a -> VisitCount -> m (WithTag t VisitCompat)
...
emitSplitNodeInitStepChecks splitNode = branch $ do
    assumeNonRErrForSplitNode splitNode
...
emitSplitNodeInductStepChecks splitNode = branch $ do
    ...
    assume1L $ pcTrivH conts.right
    assume1L $ pcTrueH conts.left
    assumeNonRErrForSplitNode splitNode
...
assumeNonRErrForSplitNode :: MonadChecks t m => SplitProofNode t () -> m ()
assumeNonRErrForSplitNode splitNode = branchRestrs $ do
...
getSplitVisitsAt :: MonadChecks t m => VisitCount -> SplitProofNode t () -> m (ByTag t (WithTag t VisitCompat))
getSplitVisitsAt vc splitNode =
    traverse (getSplitVisitAt vc) (withTags splitNode.details)

getSplitVisitAt :: MonadChecks t m => VisitCount -> WithTag t SplitProofNodeDetails -> m (WithTag t VisitCompat)
...
emitSingleLoopInductStepChecks node = branch $ do
    assume1L =<< pcTrueH <$> getSingleRevInductVisitAt (offsetVC node.n) node
    for_ [0 .. node.n - 1] $ \i ->
...
emitSingleLoopInductBaseChecks node = branch $ do
    for_ [0 .. node.n] $ \i -> branch $ do
        assume1R =<< pcTrueH <$> getSingleRevInductVisitAt (numberVC i) node
...
emitSingleLoopRevInductChecks node = branch $ do
    curr <- getSingleRevInductVisitAt (offsetVC 1) node
    cont <- getSingleRevInductVisitAt (offsetVC 2) node
    assume1R $ pcTrueH curr
    assume1R $ trueIfAt node.pred_ cont
    assumeNonRErrForSingleRevInductNode node
...
emitSingleLoopRevInductBaseChecks node = branch $ do
    cont <- getSingleRevInductVisitAt (offsetVC 1) node
    assumeSingleLoopCounterEqBound node
...
assumeSingleRevInduct node =
    assume1R =<< trueIfAt node.pred_ <$> getSingleRevInductVisitAtR (numberVC 0) node

assumeSingleLoopCounterEqBound :: MonadChecks t m => SingleRevInductProofNode t () -> m ()
assumeSingleLoopCounterEqBound node = do
    visit <- getSingleRevInductVisitAt (offsetVC 0) node
    assume1R $
        eqInductH
...
assumeNonRErrForSingleRevInductNode :: MonadChecks t m => SingleRevInductProofNode t () -> m ()
assumeNonRErrForSingleRevInductNode node = branchRestrs $ do
    restrict1L rightTag $ Restr node.point $ doubleRangeVC 1 3
    applyRestrOthers
    assume1R =<< pcFalseH <$> getVisitWithTag rightTag Err

getSingleRevInductVisitAt :: MonadChecks t m => VisitCount -> SingleRevInductProofNode t () -> m (WithTag t VisitCompat)
getSingleRevInductVisitAt = getSingleRevInductVisitAtInner True

-- HACK to match graph-refine
getSingleRevInductVisitAtR :: MonadChecks t m => VisitCount -> SingleRevInductProofNode t () -> m (WithTag t VisitCompat)
getSingleRevInductVisitAtR = getSingleRevInductVisitAtInner False

getSingleRevInductVisitAtInner :: MonadChecks t m => Bool -> VisitCount -> SingleRevInductProofNode t () -> m (WithTag t VisitCompat)
...
getLoopEqHypsAt useIfAt vc node = do
    start <- getSingleRevInductVisitAt (numberVC 0) node
    visit <- getSingleRevInductVisitAt vc node
