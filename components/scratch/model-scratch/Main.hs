...
thisLookupCheck :: ProblemAnalysis AsmRefineTag -> CheckPath -> ProofChecks' -> ProofCheck AsmRefineTag ProofCheckDescription
thisLookupCheck analysis path = view $
      #unwrap
    % expectingAt path.checkGroupPath.pairingId
    % #root
    % to (followProofScriptEdgePath path.checkGroupPath.proofScriptEdgePath)
    % to x
  where
    x :: [ProofCheck AsmRefineTag ProofCheckDescription] -> ProofCheck AsmRefineTag ProofCheckDescription
    x checks = (`genericIndex` path.indexInGroup.unwrap) $ fromJust $ lookup path.checkGroupPath.checkIndices $ prunedProofCheckGroups analysis checks
